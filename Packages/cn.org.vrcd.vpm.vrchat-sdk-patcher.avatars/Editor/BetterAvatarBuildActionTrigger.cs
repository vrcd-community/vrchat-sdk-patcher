...
namespace VRCD.VRChatPackages.VRChatSDKPatcher.Avatars.Editor
{
    ...
    {
        [MenuItem(MenuItem("VRChat SDK Patcher (No longer supported)/[Avatars] Build Actions/[Build] Only", priority = 2000))]
        private static void Build()
        {
            AvatarBuildActionUtils.BuildGuard();
        }

        [MenuItem(MenuItem("VRChat SDK Patcher (No longer supported)/[Avatars] Build Actions/[Build] and [Test]", priority = 3000))]
        private static void BuildAndTest()
        {
            AvatarBuildActionUtils.BuildAndTestGuard();
        }

        [MenuItem(MenuItem("VRChat SDK Patcher (No longer supported)/[Avatars] Build Actions/[Build] and [Upload]", priority = 4000))]
        private static void BuildAndUpload()
        {
            AvatarBuildActionUtils.BuildAndUploadGuard();
        }

        [MenuItem(MenuItem("VRChat SDK Patcher (No longer supported)/[Avatars] Build Actions/[Upload] Last Build", priority = 4000))]
